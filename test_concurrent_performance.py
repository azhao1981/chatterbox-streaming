...
def test_single_request(model, request_id, text, lock):
    ...
    # 保存音频文件
    output_file = f"concurrent_req_{request_id}.wav"
...
def main():
    parser = argparse.ArgumentParser(description='Chatterbox并发性能测试')
    parser.add_argument('--concurrency', type=int, default=3,
                       help='并发请求数 (默认: 3)')
    parser.add_argument('--rounds', type=int, default=1,
                       help='测试轮数 (默认: 1)')
    parser.add_argument('--text', type=str,
                       default="Hello world, this is a concurrent performance test for text to speech.",
                       help='测试文本')
    ...
    print("=" * 60)
    print(f"Chatterbox并发性能测试")
    print(f"并发数: {args.concurrency}, 轮数: {args.rounds}")
    print("=" * 60)

    # 执行并发测试
    test_concurrent_performance(model, args.text, args.concurrency, args.rounds)
